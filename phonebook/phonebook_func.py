#   Python Ver: 3.8.3
#
#   Author: Grady Mellin Based on a project written by Daniel A. Christie
#
#   Purpose: Phonebook demo for demonstrating OOP and using Tkinter GUI Modules
...
def addToList(self):
    ...
    #Normalize the data to keep it consistant
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            cursor = conn.cursor()
            #Check to see for existance of the full name if so we will alert the user and disregard the request
